    ...
    for (const tableName of SYSTEM_TABLES) {
        try {
            ...
            results.push({
                ...
                hasIssues: true,
                issuesCount: 1,
                issues: [`Erro ao verificar estrutura da tabela: ${err.message}`]
            });
        }
    }
...
/**
 * Diagnóstico individual por tabela
 */
async function diagnoseTable(tableName) {
    // 1. Verificar se tabela existe e obter contagem
    ...
    const rowCountRes = await db.query(`SELECT COUNT(*) as total FROM ${tableName}`);
    const totalRows = parseInt(rowCountRes.rows[0].total);
    const issues = [];

    // 2. Regras de diagnóstico específicas por tabela
    switch (tableName) {
        case 'alunos': {
            ...
            if (parseInt(invalidPresenca.rows[0].count) > 0) {
                issues.push(`${invalidPresenca.rows[0].count} aluno(s) com frequência fora da faixa permitida (0-100%).`);
            }
            ...
            if (parseInt(invalidNota.rows[0].count) > 0) {
                issues.push(`${invalidNota.rows[0].count} aluno(s) com média geral fora do intervalo (0.0 - 10.0).`);
            }

            const invalidIdade = await db.query(`
                SELECT COUNT(*) as count FROM alunos WHERE idade < 10 OR idade > 30
            `);
            if (parseInt(invalidIdade.rows[0].count) > 0) {
                issues.push(`${invalidIdade.rows[0].count} aluno(s) com idade atípica (< 10 ou > 30 anos).`);
            }
            ...
            if (parseInt(semLogin.rows[0].count) > 0) {
                issues.push(`${semLogin.rows[0].count} aluno(s) cadastrado(s) sem credenciais de acesso no portal.`);
            }
            break;
        }
        ...
        case 'alunos_login': {
            ...
            if (parseInt(orfaosLogin.rows[0].count) > 0) {
                issues.push(`${orfaosLogin.rows[0].count} conta(s) de login órfã(s) apontando para alunos inexistentes.`);
            }
            ...
            if (parseInt(duplicateMatricula.rows[0].count) > 0) {
                issues.push(`${duplicateMatricula.rows[0].count} matrícula(s) duplicada(s) detectada(s).`);
            }
            break;
        }
        ...
        case 'aluno_competencias': {
            ...
            if (parseInt(orfaosComp.rows[0].count) > 0) {
                issues.push(`${orfaosComp.rows[0].count} avaliação(ões) órfã(s) com aluno ou competência inexistente.`);
            }
            ...
            if (parseInt(invalidGrade.rows[0].count) > 0) {
                issues.push(`${invalidGrade.rows[0].count} nota(s) de competência fora do intervalo [0.0 - 10.0].`);
            }
            break;
        }
        ...
        case 'tarefas': {
            ...
            if (parseInt(orfaosCriador.rows[0].count) > 0) {
                issues.push(`${orfaosCriador.rows[0].count} tarefa(s) vinculada(s) a criador de usuário inexistente.`);
            }
            ...
            if (parseInt(orfaosCompTarefa.rows[0].count) > 0) {
                issues.push(`${orfaosCompTarefa.rows[0].count} tarefa(s) referenciando competência inexistente.`);
            }
            break;
        }
        ...
        case 'tarefas_alunos': {
            ...
            if (parseInt(orfaosTarefasAlunos.rows[0].count) > 0) {
                issues.push(`${orfaosTarefasAlunos.rows[0].count} registro(s) de tarefa atribuída órfão(s).`);
            }
            ...
            if (parseInt(invalidTaskGrade.rows[0].count) > 0) {
                issues.push(`${invalidTaskGrade.rows[0].count} nota(s) de tarefa atribuída fora do limite [0.0 - 10.0].`);
            }
            ...
            if (parseInt(invalidStatus.rows[0].count) > 0) {
                issues.push(`${invalidStatus.rows[0].count} registro(s) com status de entrega inválido.`);
            }
            break;
        }
        ...
        case 'usuarios': {
            ...
            if (parseInt(invalidCargo.rows[0].count) > 0) {
                issues.push(`${invalidCargo.rows[0].count} usuário(s) com cargo inconsistente.`);
            }
            ...
            if (parseInt(invalidStatusUser.rows[0].count) > 0) {
                issues.push(`${invalidStatusUser.rows[0].count} usuário(s) com status inconsistente.`);
            }
            break;
        }
        ...
        case 'solicitacoes_senha': {
            ...
            if (parseInt(orfaosSolicitacoes.rows[0].count) > 0) {
                issues.push(`${orfaosSolicitacoes.rows[0].count} solicitação(ões) de redefinição de usuário inexistente.`);
            }
            ...
            if (parseInt(solicitacoesAntigas.rows[0].count) > 0) {
                issues.push(`${solicitacoesAntigas.rows[0].count} solicitação(ões) pendente(s) há mais de 30 dias.`);
            }
            break;
        }
        ...
        case 'notificacoes': {
            ...
            if (parseInt(orfaosNotificacoes.rows[0].count) > 0) {
                issues.push(`${orfaosNotificacoes.rows[0].count} notificação(ões) órfã(s) ou sem destinatário válido.`);
            }
            break;
        }
        ...
        case 'configuracoes_notificacoes': {
            ...
            if (parseInt(orfaosConfig.rows[0].count) > 0) {
                issues.push(`${orfaosConfig.rows[0].count} configuração(ões) de notificação de usuário/aluno inexistente.`);
            }
            break;
        }
        ...
        case 'ia_conversas': {
            ...
            if (parseInt(orfaosConversas.rows[0].count) > 0) {
                issues.push(`${orfaosConversas.rows[0].count} conversa(s) de IA órfã(s).`);
            }
            break;
        }
        ...
        case 'ia_mensagens': {
            ...
            if (parseInt(orfaosMensagens.rows[0].count) > 0) {
                issues.push(`${orfaosMensagens.rows[0].count} mensagem(ns) de IA pertencentes a conversas inexistentes.`);
            }
            break;
        }
    }
...
async function repairSystem(targetTable = 'all') {
    const tablesToRepair = targetTable === 'all' 
        ? SYSTEM_TABLES 
        : SYSTEM_TABLES.filter(t => t === targetTable);
    ...
    try {
        ...
        for (const tableName of tablesToRepair) {
            switch (tableName) {
                case 'alunos': {
                    // 1. Ajustar frequência para [0 - 100]
                    ...
                    if (fixPresenca.rowCount > 0) {
                        summary.push(`[alunos] ${fixPresenca.rowCount} registro(s) de presença ajustado(s) para o limite legal (0-100%).`);
                    }

                    // 2. Ajustar média para [0.0 - 10.0]
                    ...
                    if (fixNota.rowCount > 0) {
                        summary.push(`[alunos] ${fixNota.rowCount} média(s) fora do intervalo ajustada(s) para valores entre 0.0 e 10.0.`);
                    }

                    // 3. Gerar credenciais para alunos sem login
                    ...
                    if (semLoginRes.rows.length > 0) {
                        for (const aluno of semLoginRes.rows) {
                            ...
                            const emailPrefix = aluno.nome.toLowerCase().replace(/[^a-z0-9]/g, '.').replace(/\.+/g, '.').slice(0, 30);
                        ...
                        summary.push(`[alunos_login] Geradas ${semLoginRes.rows.length} nova(s) credencial(is) de acesso para aluno(s) sem login.`);
                    }
                    break;
                }

                case 'alunos_login': {
                    // Remover logins de alunos que foram excluídos
                    ...
                    if (delOrfaosLogin.rowCount > 0) {
                        summary.push(`[alunos_login] ${delOrfaosLogin.rowCount} conta(s) de login órfã(s) sem aluno associado foram removida(s).`);
                    }
                    break;
                }

                case 'aluno_competencias': {
                    // 1. Remover avaliações órfãs
                    ...
                    if (delOrfaosComp.rowCount > 0) {
                        summary.push(`[aluno_competencias] ${delOrfaosComp.rowCount} avaliação(ões) órfã(s) removida(s).`);
                    }

                    // 2. Corrigir notas fora da faixa
                    ...
                    if (fixNotasComp.rowCount > 0) {
                        summary.push(`[aluno_competencias] ${fixNotasComp.rowCount} nota(s) de competência corrigida(s) para o limite [0.0 - 10.0].`);
                    }
                    break;
                }

                case 'tarefas': {
                    // Desvincular criadores ou competências inexistentes
                    ...
                    if (fixCriador.rowCount > 0) {
                        summary.push(`[tarefas] ${fixCriador.rowCount} tarefa(s) com criador inexistente foram desvinculadas com segurança.`);
                    }
                    ...
                    if (fixCompTarefa.rowCount > 0) {
                        summary.push(`[tarefas] ${fixCompTarefa.rowCount} tarefa(s) com competência inexistente foram desvinculadas.`);
                    }
                    break;
                }

                case 'tarefas_alunos': {
                    // Remover entregas órfãs
                    ...
                    if (delOrfaosTA.rowCount > 0) {
                        summary.push(`[tarefas_alunos] ${delOrfaosTA.rowCount} registro(s) de tarefa atribuída órfão(s) excluído(s).`);
                    }

                    // Corrigir notas fora do intervalo
                    ...
                    if (fixTaskNotas.rowCount > 0) {
                        summary.push(`[tarefas_alunos] ${fixTaskNotas.rowCount} nota(s) de tarefa ajustada(s) para a faixa válida [0.0 - 10.0].`);
                    }

                    // Corrigir status inválidos para 'PENDENTE'
                    ...
                    if (fixStatus.rowCount > 0) {
                        summary.push(`[tarefas_alunos] ${fixStatus.rowCount} status corrompido(s) redefinido(s) para 'PENDENTE'.`);
                    }
                    break;
                }
                ...
                case 'usuarios': {
                    ...
                    if (fixCargo.rowCount > 0) {
                        summary.push(`[usuarios] ${fixCargo.rowCount} usuário(s) com cargo inválido foram redefinidos para 'Professor'.`);
                    }
                    ...
                    if (fixStatusU.rowCount > 0) {
                        summary.push(`[usuarios] ${fixStatusU.rowCount} usuário(s) com status corrompido foram ajustados para 'ATIVO'.`);
                    }
                    break;
                }
                ...
                case 'solicitacoes_senha': {
                    ...
                    if (delOrfaosSol.rowCount > 0) {
                        summary.push(`[solicitacoes_senha] ${delOrfaosSol.rowCount} solicitação(ões) de usuário inexistente foram removidas.`);
                    }
                    ...
                    if (rejAntigas.rowCount > 0) {
                        summary.push(`[solicitacoes_senha] ${rejAntigas.rowCount} solicitação(ões) pendente(s) há mais de 30 dias foram encerradas.`);
                    }
                    break;
                }
                ...
                case 'notificacoes': {
                    ...
                    if (delNotif.rowCount > 0) {
                        summary.push(`[notificacoes] ${delNotif.rowCount} notificação(ões) órfã(s) removida(s).`);
                    }
                    break;
                }
                ...
                case 'configuracoes_notificacoes': {
                    ...
                    if (delCfg.rowCount > 0) {
                        summary.push(`[configuracoes_notificacoes] ${delCfg.rowCount} configuração(ões) de notificação órfã(s) excluída(s).`);
                    }
                    break;
                }
                ...
                case 'ia_conversas': {
                    ...
                    if (delConv.rowCount > 0) {
                        summary.push(`[ia_conversas] ${delConv.rowCount} conversa(s) de IA órfã(s) removida(s).`);
                    }
                    break;
                }
                ...
                case 'ia_mensagens': {
                    ...
                    if (delMsg.rowCount > 0) {
                        summary.push(`[ia_mensagens] ${delMsg.rowCount} mensagem(ns) de IA órfã(s) excluída(s).`);
                    }
                    break;
                }
            }
        ...
        if (summary.length === 0) {
            summary.push('Nenhuma anomalia ou dado corrompido necessitava de intervenção. A base está completamente íntegra.');
        }

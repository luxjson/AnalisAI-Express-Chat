(function () {
  ...
  function renderMath(value) {
    let formula = value
      ...
      .replace(
        /\\frac\{([^{}]+)\}\{([^{}]+)\}/g,
        '<span class="ai-fraction"><span>$1</span><span>$2</span></span>',
      )
      .replace(/\\div/g, "÷")
      .replace(/\\times/g, "×")
      .replace(/\\cdot/g, "·")
  ...
  function addMessage(text, type) {
    ...
    if (type === "assistant" || type === "system")
      element.innerHTML = renderAssistantText(text);
  ...
  function addGradeAction(action) {
    ...
    element.innerHTML =
      '<div><strong>Confirmar alteração?</strong><span></span></div><button type="button"><i class="fas fa-check"></i> Aplicar</button>';
    element.querySelector("span").textContent =
      `${action.alunoNome} · ${action.competenciaNome} · nota ${action.nota}`;
    element.querySelector("button").addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (!response.ok)
          throw new Error(data.error || "Não foi possível alterar a nota.");
  ...
  function setMessages(conversationMessages) {
    ...
    conversationMessages.forEach((message) =>
      addMessage(message.text, message.role === "model" ? "assistant" : "user"),
    );
  }
  ...
  function renderHistory(conversations) {
    ...
    if (!conversations.length) {
      historyList.innerHTML =
        '<div class="ai-history-empty"><p>Nenhuma conversa salva ainda.</p></div>';
      return;
    }
    conversations.forEach((conversation) => {
      ...
      item.querySelectorAll("[data-action]").forEach((actionButton) =>
        actionButton.addEventListener("click", async (event) => {
          ...
          if (
            action === "apagar" &&
            !window.confirm("Apagar esta conversa e todas as mensagens?")
          )
            return;
          actionButton.disabled = true;
          try {
            const response = await csrfFetch(
              `/api/ia/historico/${conversation.id}`,
              {
                method: "PATCH",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (!response.ok)
              throw new Error(
                data.error || "Não foi possível atualizar a conversa.",
              );
            if (action === "apagar") {
              item.remove();
              if (conversationId === conversation.id)
                newChat().catch((error) => addMessage(error.message, "error"));
  ...
  async function openHistory() {
    ...
    historyList.innerHTML =
      '<div class="ai-history-loading"><span></span><span></span><span></span></div>';
    try {
  ...
  chat
    ...
    .forEach((element) => element.addEventListener("click", closeHistory));

(function () {
    ...
    function renderMarkdown(value) {
        ...
        function flushList() {
            if (currentList) { out.push(`</${currentList}>`); currentList = null; }
        }
        for (let i = 0; i < lines.length; i++) {
            let line = lines[i];
            let trimmed = line.trim();
            if (!trimmed) { flushParagraph(); flushList(); continue; }
            if (/^([*_-][ \t]*){3,}$/.test(trimmed)) { flushParagraph(); flushList(); out.push('<hr>'); continue; }
            const decoMatch = trimmed.match(/^((?:[*_-]{3,}[ \t]*)+)(.+)$/);
            if (decoMatch) { flushParagraph(); flushList(); out.push('<hr>'); trimmed = decoMatch[2].trim(); line = trimmed; }
            ...
            if (ulMatch) {
                flushParagraph();
                if (currentList !== 'ul') { flushList(); out.push('<ul>'); currentList = 'ul'; }
            ...
            if (olMatch) {
                flushParagraph();
                if (currentList !== 'ol') { flushList(); out.push('<ol>'); currentList = 'ol'; }
                out.push(`  <li value="${olMatch[2]}">${formatInline(olMatch[3])}</li>`);
                continue;
            }
            flushList();
    ...
    button.addEventListener('click', () => analyze('Faça um diagnóstico pedagógico completo em texto corrido sobre este aluno. Analise situação atual, pontos fortes, prioridades de intervenção e próximos passos práticos.'));

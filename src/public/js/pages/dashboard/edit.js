  ...
  reader.onload = async function (e) {
    try {
      ...
      if (linhaAlunos === -1) {
        openErrorModal(
          "FORMATO INVÁLIDO",
          "Formato de planilha não reconhecido. Use o modelo baixado.",
          "",
        );
        return;
      }
      ...
      for (let i = linhaAlunos + 1; i < linhas.length; i++) {
        ...
        if (
          primeiraCelula.includes("HISTÓRICO") ||
          primeiraCelula.includes("INSTRUÇÕES")
        ) {
          break;
        }
        ...
        if (anoEscolar && !anosValidos.includes(anoEscolar)) {
          openErrorModal(
            "ANO ESCOLAR INVÁLIDO",
            `"${anoEscolar}" não é permitido`,
            "Use: 1º MÉDIO, 2º MÉDIO, 3º MÉDIO, 9º FUNDAMENTAL",
          );
          return;
        }
        ...
        if (idade < 10 || idade > 20) {
          openErrorModal(
            ...
            "A idade deve estar entre 10 e 20 anos",
          );
          return;
        }
        ...
        if (linha[5]) {
          ...
          if (presenca < 0 || presenca > 100) {
            openErrorModal(
              "PRESENÇA INVÁLIDA",
              `Presença ${presenca}% inválida`,
              "A presença deve estar entre 0% e 100%",
            );
            return;
          }
        }
      ...
      if (alunosParaImportar.length === 0) {
        openErrorModal(
          "NENHUM ALUNO",
          "Nenhum aluno encontrado na planilha!",
          "",
        );
        return;
      }
...
document
  .getElementById("competenciasSearchInput")
...
document
  .getElementById("chartSearchInput")
  .addEventListener("keyup", function () {
    const searchTerm = this.value.toLowerCase();
...
function closeAddModal() {
  document.getElementById("addAlunoModal").style.display = "none";
}
...
function carregarCompetencias(alunoId) {
  fetch(`/dashboard/competencias-aluno/${alunoId}`)
    ...
    .then((data) => {
      ...
      if (data.length === 0) {
        container.innerHTML =
          '<p style="color: #888; text-align: center;">Nenhuma competência registrada ainda.</p>';
      } else {
        let html = "";
        data.forEach((comp) => {
          const notaClass =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  dataAlunos.forEach((aluno) => {
    ...
    if (!canvas || !aluno.competencias || aluno.competencias.length === 0)
      return;
...
function applyFilter() {
  ...
  document
    .querySelectorAll(".chart-card, .competencia-card")
    .forEach((card) => {
      const nomeAluno =
        card.querySelector("h3")?.textContent.replace("ALUNO:", "").trim() ||
        "";
      let encontrou = false;
...
function clearFilters() {
  ...
  document
    .querySelectorAll("#alunosTableBody tr, .chart-card, .competencia-card")
...
document.getElementById("searchInput").addEventListener("keyup", function () {
  ...
  document.querySelectorAll("#alunosTableBody tr").forEach((row) => {
    row.style.display = row.cells[1].textContent.toUpperCase().includes(filter)
      ? ""
      : "none";
  });
});
...
async function exportarTabelaParaExcel() {
  ...
  dataAlunos.forEach((aluno, index) => {
    ...
    if (aluno.competencias && aluno.competencias.length > 0) {
      ...
      competenciasStr = aluno.competencias
        .map(
          (comp) =>
  ...
  dataAlunos.forEach((aluno) => {
    ...
    titleRow.getCell(1).value =
      `HISTÓRICO INDIVIDUAL: ${aluno.nome.toUpperCase()}`;

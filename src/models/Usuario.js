...
class Usuario {
  static async findByEmail(email) {
    const result = await db.query(
      "SELECT id, nome, email, senha, cargo, status, data_criacao, ultimo_acesso FROM usuarios WHERE email = $1",
      [email.toLowerCase()],
    );
  ...
  static async findById(id) {
    const result = await db.query(
      "SELECT id, nome, email, cargo, status, data_criacao FROM usuarios WHERE id = $1",
      [id],
    );
    return result.rows[0];
  }
  ...
  static async findAdmins() {
    const result = await db.query("SELECT id FROM usuarios WHERE cargo = $1", [
      "Admin",
    ]);
    return result.rows;
  }

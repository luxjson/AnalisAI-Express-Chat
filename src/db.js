...
if (production && allowInsecureTls)
  throw new Error(
    "DB_SSL_REJECT_UNAUTHORIZED=false não é permitido em produção.",
  );
...
const pool = new Pool({
  ...
  max: 3, // Reduzido para evitar estourar o limite do Neon (plano gratuito = 10 conexões)
  idleTimeoutMillis: 10_000, // Libera conexões ociosas após 10 segundos
  connectionTimeoutMillis: 5_000, // Tempo máximo para estabelecer conexão
  statement_timeout: 15_000, // Timeout para cada statement SQL
  query_timeout: 15_000, // Timeout para consultas completas
  ...
  runtimeSchemaPromise = (async () => {
    ...
    const savepoint = async (sql, params = []) => {
      ...
      try {
        ...
        await client
          .query("RELEASE SAVEPOINT runtime_schema_step")
          .catch(() => {});
      }
    };
    try {
      ...
      `);
      await savepoint(
        `DELETE FROM configuracoes_notificacoes a USING configuracoes_notificacoes b WHERE a.id < b.id AND a.usuario_id IS NOT NULL AND a.usuario_id = b.usuario_id`,
      );
      await savepoint(
        `DELETE FROM configuracoes_notificacoes a USING configuracoes_notificacoes b WHERE a.id < b.id AND a.aluno_id IS NOT NULL AND a.aluno_id = b.aluno_id`,
      );
      await savepoint(
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_config_notif_usuario ON configuracoes_notificacoes(usuario_id) WHERE usuario_id IS NOT NULL",
      );
      await savepoint(
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_config_notif_aluno ON configuracoes_notificacoes(aluno_id) WHERE aluno_id IS NOT NULL",
      );
      ...
      `);
      await savepoint(
        `DELETE FROM security_rate_limits WHERE window_start < CURRENT_TIMESTAMP - INTERVAL '48 hours'`,
      );
      ...
      await client.query(
        "CREATE TABLE IF NOT EXISTS system_backups (id BIGSERIAL PRIMARY KEY)",
      );
      ...
      const addColumn = async (name, definition) => {
        if (!columns.has(name)) {
          const result = await savepoint(
            `ALTER TABLE system_backups ADD COLUMN ${name} ${definition}`,
          );
      ...
      if (columns.has("reason")) {
        await savepoint(
          "UPDATE system_backups SET reason = COALESCE(NULLIF(reason, ''), 'migracao') WHERE reason IS NULL OR reason = ''",
        );
        await savepoint(
      ...
      if (columns.has("size_bytes") && columns.has("content")) {
        await savepoint(
          "UPDATE system_backups SET size_bytes = octet_length(content::text) WHERE (size_bytes IS NULL OR size_bytes = 0) AND content IS NOT NULL",
        );
      }
      ...
      if (checksumColumn) {
        const checksumExpr = [
          ...
          "character varying",
          "character",
        ].includes(checksumColumn.data_type)
          ? "''"
        ...
        if (checksumExpr && columns.has("content")) {
          await savepoint(
            `UPDATE system_backups SET checksum = ${checksumExpr} WHERE checksum IS NULL AND content IS NOT NULL`,
          );
        }
      }
      ...
      const finalChecksum = finalColumns.rows.find(
        (r) => r.column_name === "checksum",
      );
      ...
      if (finalNames.has("filename")) {
        ...
        await savepoint(
          "ALTER TABLE system_backups ALTER COLUMN filename SET NOT NULL",
        );
        await savepoint(
      ...
      if (finalChecksum && finalChecksum.is_nullable === "NO") {
        const remaining = await client.query(
          "SELECT COUNT(*)::int AS count FROM system_backups WHERE checksum IS NULL",
        );
        if (remaining.rows[0].count > 0) {
          console.warn(
            `[Schema] system_backups.checksum ainda possui ${remaining.rows[0].count} registro(s) nulo(s); backups novos continuam fornecendo checksum.`,
          );
        }
      }

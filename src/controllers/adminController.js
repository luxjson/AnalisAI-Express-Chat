...
exports.atualizarUsuario = async (req, res) => {
    ...
    const userEmail = email(req.body.email);
    const cargo = ALLOWED_CARGOS.includes(req.body.cargo) ? req.body.cargo : null;
    const status = ALLOWED_STATUS.includes(req.body.status) ? req.body.status : null;
    ...
    if (id === req.session.userId && (status !== 'ATIVO' || cargo !== 'Admin')) {
        req.flash('error_msg', 'Você não pode desativar ou remover seu próprio acesso de administrador.');
        return res.redirect('/dashboard/usuarios');
    }
    try {
        const current = await db.query('SELECT cargo, status FROM usuarios WHERE id = $1', [id]);
        if (current.rows.length === 0) {
            req.flash('error_msg', 'Usuário não encontrado.');
            return res.redirect('/dashboard/usuarios');
        }
        const roleChanged = current.rows[0].cargo !== cargo || current.rows[0].status !== status;
        await db.query(
        ...
        if (process.env.NODE_ENV === 'production' && roleChanged) {
            await db.query(`DELETE FROM web_sessions WHERE sess->>'userId' = $1`, [String(id)]);
        }
...
exports.deletarUsuario = async (req, res) => {
    ...
    try {
        const target = await db.query('SELECT cargo FROM usuarios WHERE id = $1', [id]);
        if (target.rows[0]?.cargo === 'Admin') {
            const admins = await db.query("SELECT COUNT(*) FROM usuarios WHERE cargo = 'Admin' AND status = 'ATIVO'");
            if (Number(admins.rows[0].count) <= 1) {
                req.flash('error_msg', 'O último administrador ativo não pode ser removido.');
                return res.redirect('/dashboard/usuarios');
            }
        }
...
exports.aprovarSolicitacaoSenha = async (req, res) => {
    ...
    if (!solicitacaoId || !password(nova_senha)) {
    ...
    const client = await db.connect();
        ...
        'SELECT * FROM solicitacoes_senha WHERE id = $1 AND status = $2 FOR UPDATE',
    ...
    if (userLock.rows.length === 0) {
        ...
        if (process.env.NODE_ENV === 'production') {
            await client.query(`DELETE FROM web_sessions WHERE sess->>'userId' = $1`, [String(s.usuario_id)]);
        }
        await client.query(
        ...
        await criarNotificacao('senha_alterada', s.usuario_id, null, 'Senha Redefinida', 'Sua senha foi redefinida por um administrador', '/login', 'fas fa-check-circle', '#217346');

...
const { text, integerId, numberInRange, ALLOWED_PRIORITY, ALLOWED_TASK_STATUS, ALLOWED_YEARS } = require('../utils/validation');

exports.listar = async (req, res) => {
    try {
        const turmaFilter = ALLOWED_YEARS.includes(req.query.turma) ? req.query.turma : '';
        const statusFilter = ALLOWED_TASK_STATUS.includes(req.query.status) ? req.query.status : '';
        let query = `
            ...
            WHERE 1=1`;
        const params = [];
        if (turmaFilter) { params.push(turmaFilter); query += ` AND t.turma = $${params.length}`; }
        if (statusFilter) { params.push(statusFilter); query += ` AND t.status = $${params.length}`; }
        query += ` GROUP BY t.id, u.nome ORDER BY CASE WHEN t.status = 'ATIVA' THEN 1 ELSE 2 END, t.data_criacao DESC`;

        const [tarefasResult, alunosResult, statsResult, competenciasResult] = await Promise.all([
...
exports.criar = async (req, res) => {
    ...
    const data_entrega = typeof req.body.data_entrega === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(req.body.data_entrega) ? req.body.data_entrega : null;
    const prioridade = ALLOWED_PRIORITY.includes(req.body.prioridade) ? req.body.prioridade : 'MEDIA';
    const competencia_id = req.body.competencia_id ? integerId(req.body.competencia_id) : null;
    const alunos = Array.isArray(req.body.alunos) ? req.body.alunos.map(integerId).filter(Boolean).slice(0, 500) : [];
    if (!titulo || !turma) {
    ...
    try {
        ...
        const tarefaResult = await client.query(
            ...
            [titulo, descricao, turma, data_entrega, prioridade, competencia_id, req.session.userId]
        );
        ...
        for (const alunoId of alunosParaAtribuir) {
            await client.query(
                `INSERT INTO tarefas_alunos (tarefa_id, aluno_id, status) VALUES ($1, $2, 'PENDENTE') ON CONFLICT (tarefa_id, aluno_id) DO NOTHING`,
                [tarefaId, alunoId]
            );
            notificationAlunoIds.push(alunoId);
        }
        await client.query('COMMIT');

        for (const alunoId of notificationAlunoIds) {
            await criarNotificacao('tarefa', null, alunoId, 'Nova Tarefa', `Você recebeu uma nova tarefa: ${titulo}`, '/aluno/tarefas', 'fas fa-tasks', '#217346');
        }
...
exports.avaliarAluno = async (req, res) => {
    ...
    if (!tarefa_id || !aluno_id || nota === null) return res.status(400).json({ success: false, error: 'Dados de avaliação inválidos.' });
    ...
    try {
        ...
        if (tarefa.rows[0]?.competencia_id) {
            ...
            if (media.rows[0].media) await client.query('UPDATE alunos SET nota = $1 WHERE id = $2', [media.rows[0].media, aluno_id]);
        }
        await client.query('COMMIT');
        await criarNotificacao('avaliacao', null, aluno_id, 'Tarefa Avaliada', `Sua tarefa foi avaliada com nota ${nota}`, '/aluno/tarefas', 'fas fa-star', '#217346');
...
exports.devolverAluno = async (req, res) => {
    const tarefa_id = integerId(req.body.tarefa_id);
    const aluno_id = integerId(req.body.aluno_id);
    if (!tarefa_id || !aluno_id) return res.status(400).json({ success: false, error: 'Dados inválidos.' });
    try {
        ...
        if (result.rows.length === 0) return res.status(404).json({ success: false, error: 'Entrega não encontrada.' });
        await criarNotificacao('devolucao', null, aluno_id, 'Tarefa Devolvida', 'Sua tarefa foi devolvida para correção', '/aluno/tarefas', 'fas fa-undo-alt', '#ffa500');
...
exports.obterDetalhes = async (req, res) => {
    ...
    try {
        const tarefaResult = await db.query(`SELECT t.*, c.nome as competencia_nome, u.nome as professor_nome
            FROM tarefas t LEFT JOIN competencias c ON t.competencia_id = c.id LEFT JOIN usuarios u ON t.criado_por = u.id WHERE t.id = $1`, [tarefaId]);
...
exports.editar = async (req, res) => {
    ...
    const descricao = text(req.body.descricao, 5000) || null;
    const data_entrega = typeof req.body.data_entrega === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(req.body.data_entrega) ? req.body.data_entrega : null;
    const prioridade = ALLOWED_PRIORITY.includes(req.body.prioridade) ? req.body.prioridade : null;
    const status = ALLOWED_TASK_STATUS.includes(req.body.status) ? req.body.status : null;
    if (!tarefaId || !titulo || !prioridade || !status) {
        req.flash('error_msg', 'Dados da tarefa inválidos.');
        return res.redirect('/dashboard/tarefas');
    }
    try {
        await db.query(`UPDATE tarefas SET titulo = $1, descricao = $2, data_entrega = $3, prioridade = $4, status = $5, data_atualizacao = CURRENT_TIMESTAMP WHERE id = $6`,
...
exports.atualizarStatusAluno = async (req, res) => {
    const tarefa_id = integerId(req.body.tarefa_id);
    const aluno_id = integerId(req.body.aluno_id);
    const status = ['PENDENTE', 'ENTREGUE', 'CONCLUIDA', 'DEVOLVIDA', 'ATRASADA'].includes(req.body.status) ? req.body.status : null;
    const observacoes = text(req.body.observacoes, 2000) || null;
    if (!tarefa_id || !aluno_id || !status) return res.status(400).json({ error: 'Dados inválidos' });
    try {
        await db.query(`UPDATE tarefas_alunos SET status = $1, data_entrega = CASE WHEN $1 = 'CONCLUIDA' THEN CURRENT_TIMESTAMP ELSE data_entrega END WHERE tarefa_id = $2 AND aluno_id = $3`, [status, tarefa_id, aluno_id]);
        res.json({ success: true });
    } catch (err) {
...
exports.estatisticas = async (req, res) => {
    try {
        const result = await db.query(`SELECT COUNT(*) as total, COUNT(CASE WHEN status = 'ATIVA' THEN 1 END) as ativas,
            COUNT(CASE WHEN data_entrega < CURRENT_DATE AND status = 'ATIVA' THEN 1 END) as atrasadas,

...
const { integerId, text, ALLOWED_YEARS } = require('../utils/validation');
...
exports.getConta = async (req, res) => {
    try {
        ...
        const formatDate = (value) => value ? new Date(value).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' }) : '—';
        ...
        if (req.session.aluno?.id) {
            const result = await db.query(`
                ...
                WHERE a.id = $1 LIMIT 1`, [req.session.aluno.id]);
            if (!result.rows.length) return res.status(404).json({ error: 'Aluno não encontrado' });
            const row = result.rows[0];
            return res.json({
                initial: row.nome?.charAt(0).toUpperCase() || 'A', name: row.nome, class: row.ano_escolar,
                registration: row.matricula || '—', email: row.email || 'Não informado', age: row.idade ? `${row.idade} anos` : '—',
                assessments: Number(row.avaliacoes || 0), attendance: `${Number(row.presenca || 0)}%`, status: row.status || 'ATIVO',
                created: formatDate(row.data_criacao), lastAccess: formatDateTime(row.ultimo_acesso)
            });
        }

        if (req.session.userId) {
            const result = await db.query(`SELECT nome, email, cargo, data_criacao, ultimo_acesso FROM usuarios WHERE id = $1 LIMIT 1`, [req.session.userId]);
            ...
            try {
                const stats = await db.query(`SELECT
                    ...
                    (SELECT COUNT(DISTINCT ta.aluno_id)
                      ...
                      WHERE t.criado_por = $1) AS alunos`, [req.session.userId]);
                tasks = Number(stats.rows[0]?.tarefas || 0);
                students = Number(stats.rows[0]?.alunos || 0);
            } catch (statsError) {
                console.error('Erro ao buscar estatísticas da conta:', statsError.message);
                try {
                    const taskCount = await db.query('SELECT COUNT(*) AS total FROM tarefas WHERE criado_por = $1', [req.session.userId]);
                    tasks = Number(taskCount.rows[0]?.total || 0);
                } catch (_) {}
            }
            return res.json({
                initial: row.nome?.charAt(0).toUpperCase() || 'P', name: row.nome, email: row.email || 'Não informado', role: row.cargo || req.session.userCargo || 'Professor',
                created: formatDate(row.data_criacao), lastAccess: formatDateTime(row.ultimo_acesso),
                tasks, students
            });
        }
...
exports.getConfiguracoesNotificacoes = async (req, res) => {
    try {
        ...
        if (req.session.aluno) {
            ownerColumn = 'aluno_id';
            ownerId = req.session.aluno.id;
        } else if (req.session.user) {
            ownerColumn = 'usuario_id';
        ...
        return res.status(500).json({ error: 'Erro ao buscar configurações de notificações' });
    }
};
...
exports.saveConfiguracoesNotificacoes = async (req, res) => {
    try {
        ...
        const toBoolean = (value, fallback = false) => {
            ...
            if (typeof value === 'string') return ['true', '1', 'on', 'sim'].includes(value.toLowerCase());
            if (typeof value === 'number') return value === 1;
            return fallback;
        };

        const config = {
            notificacoes_ativas: toBoolean(req.body?.notificacoes_ativas, true),
            notificacoes_email: toBoolean(req.body?.notificacoes_email, false),
            notificacoes_tarefas: toBoolean(req.body?.notificacoes_tarefas, true),
            notificacoes_avaliacoes: toBoolean(req.body?.notificacoes_avaliacoes, true),
            notificacoes_competencias: toBoolean(req.body?.notificacoes_competencias, true)
        };
        let result = await db.query(
            `UPDATE configuracoes_notificacoes
             SET notificacoes_ativas = $1,
                 notificacoes_email = $2,
                 notificacoes_tarefas = $3,
                 notificacoes_avaliacoes = $4,
            ...
            [
                ...
                config.notificacoes_tarefas,
                config.notificacoes_avaliacoes,
                config.notificacoes_competencias,
                ownerId
            ]
        );

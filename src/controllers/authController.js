...
const DUMMY_PASSWORD_HASH =
  "$2b$12$C6UzMDM.H6dfI/f/IKcEe.0Jw9h7vQ8zL8VxQ5h6X7m0l0JQ5JY7K";
...
exports.loginProfessor = async (req, res) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production")
      console.error("Erro no login do professor:", err.message);
    return renderLogin(
      req,
      res,
      tipo,
...
exports.loginAluno = async (req, res) => {
  const matricula =
    typeof req.body.matricula === "string"
      ? req.body.matricula.trim().toUpperCase()
      : "";
  ...
  try {
    ...
    const senhaValida = await bcrypt.compare(
      senha,
      aluno?.senha || DUMMY_PASSWORD_HASH,
    );
    ...
    if (process.env.NODE_ENV !== "production")
      console.error("Erro no login do aluno:", err.message);
    return renderLogin(
      req,
      res,
      tipo,
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      const admins = await db.query(
        "SELECT id FROM usuarios WHERE cargo = $1 AND status = $2",
        ["Admin", "ATIVO"],
      );
    ...
    req.flash(
      "success_msg",
      "Se o e-mail estiver cadastrado, a solicitação será analisada por um administrador.",
    );
    return res.render("esqueciSenha", {
      error_msg: null,
      success_msg: req.flash("success_msg"),
